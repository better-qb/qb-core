...
function QBCore.Player.GetOfflinePlayer(id)
    if type(id) == 'string' then
        local PlayerData = MySQL.prepare.await('SELECT * FROM players where citizenid = ?', { id })
        if PlayerData then
            PlayerData.money = json.decode(PlayerData.money)
            PlayerData.job = json.decode(PlayerData.job)
            PlayerData.gang = json.decode(PlayerData.gang)
...
function QBCore.Player.CreatePlayer(PlayerData, Offline)
    ...
    function self.Functions.SetPermission(permission)
        self.PlayerData.staff = { aduty = false, level = permission, label = QBCore.Config.Staff[permission] or 'Unknown' }
    end 
